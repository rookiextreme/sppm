...
class KehadiranController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				'actions'=>array('index','view','admin','delete','create','update','pengesahan','reportkehadiran','ajaxsenaraipeserta','ajaxreportkehadiran','pesertamajlis','daftar','ajaxattendanceupdate', 'ajaxattendanceupdate2', 'reportlawatan', 'ajaxreportlawatan', 'exportexcel'),
	...
	public function actionReportLawatan()
	{
		$model = new Majlis;
		...
		$this->render('senaraitapaklawatan',array(
			'model'=>$model,
		));
	}
	/**
	...
	public function actionAjaxReportLawatan()
	{
        $id = $_POST['Majlis']['id'];
        $criteria = new CDbCriteria();
        $criteria->condition = "id_majlis = :id_majlis AND (is_bin IS NULL OR is_bin = :is_bin)";
        ...
        $criteriap->condition = "id_majlis = :id_majlis AND (is_bin IS NULL OR is_bin = :is_bin) AND id_lawatan IS NOT NULL";
        $criteriap->params = array(":id_majlis" => $id, ":is_bin" => Utils::NOT_DELETED);
		$criteriap->order = 'id_lawatan ASC';
        $jadual = Jadual::model()->findAll($criteria);
        $peserta = Peserta::model()->findAll($criteriap);
        ...
        //echo var_dump($peserta);die();
		$this->renderPartial('reportlawatan',array(
			'data'=>$data,
            'jadual'=>$jadual,
            'peserta'=>$peserta,
			'id'=> $id,
	...
	public function actionExportExcel()
	{
        ...
        //echo var_dump($peserta);die();
		$this->renderPartial('_lawatan_excel',array(
			'data'=>$data,
            'jadual'=>$jadual,
            'peserta'=>$peserta,
			'id'=> $id,
